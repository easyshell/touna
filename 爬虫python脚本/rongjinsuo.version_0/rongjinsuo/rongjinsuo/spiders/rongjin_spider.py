...
class RongJinSuoSpider(scrapy.Spider):
	name = 'rongjinsuo'
	start_urls = []
	handle_httpstatus_list = [301, 302, 400]

	def __init__(self, seed_file=""):
		#self.seed_file = "/home/lznwt/crawl/rongjinsuo/gettoubiaourls/toubiao_urls.rongjinsuo"
		self.seed_file = seed_file

	def start_requests(self):
		#test_url = "https://www.rjs.com/invest/59812.html"
		#test_url = "https://www.rjs.com/invest/53004.html"
		#yield Request(url=test_url, callback=self.parse_toubiao)
		#pass
		with codecs.open(self.seed_file, 'r', 'utf-8') as f:
			for line in f:
				link = line.strip()
				if not link: 
					continue
				if not (link.startswith('http') or link.startswith('https')):
					continue
				yield Request(url=link, callback=self.parse_toubiao)
				
	def parse_toubiao(self, response):
		print("parse_rongdiya_toubiao" + response.url)
		sel = Selector(text=response.body_as_unicode())
		rongdiya_info = RongJinSuoItem()
		rongdiya_info['product_classify'] = ""
		#print(rongdiya_info['product_classify'])
		rongdiya_info['url'] = response.url.decode('ascii')
		rongdiya_info['title'] = sel.xpath('//div[@class="pvl_tit"]/h1/text()').extract()[0]
		print(sel.xpath('//div[@class="pvl_tit"]/h1/text()').extract())
